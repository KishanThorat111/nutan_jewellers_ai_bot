...
#     """Generates a summary of the cart and calculates the subtotal."""
#     if not cart:
#         return "Your selection is currently empty. 💎", 0.0
#     summary = "💎 *Your Current Selection:*\n\n"
#     total_price = 0.0
#     for item_name, details in cart.items():
#         item_total = details['price'] * details['quantity']
#         summary += f"• {details['quantity']}x {item_name} = *£{item_total:.2f}*\n"
...
#     }
#     categories = list(menu.keys())
#     keyboard = [[InlineKeyboardButton(f"{emoji_map.get(cat.upper(), '⭐')} {cat}", callback_data=f"cat_{cat}")] for cat in categories]
#     keyboard.append([InlineKeyboardButton("💎 View Selection & Checkout", callback_data="view_cart")])
#     return InlineKeyboardMarkup(keyboard)
...
#         ])
#         await update.message.reply_text(
#             f"Welcome back, *{customer['full_name']}*! 🙏\n\nShould we use your saved address for this secure delivery?\n\n📍 _{customer['address']}_",
#             reply_markup=reply_markup, parse_mode='Markdown'
#         )
#         print(f"LOG: Returning customer {user_id} found. Awaiting address confirmation.")
#         return config.CONFIRMING_ADDRESS
...
#         context.user_data['cart'] = {}
#         start_message = (
#             f"💎 🙏 Welcome to *Nutan Jewellers*🙏 ✨\n\n"
#             "Your personal AI concierge for precious metal investments.\n\n"
#             "To begin, please provide your *full name* 👤 and *mobile number* 📱."
#         )
#         await update.message.reply_text(start_message, parse_mode='Markdown')
#         print(f"LOG: New customer {user_id}. Transitioning to GETTING_NAME_AND_PHONE.")
#         return config.GETTING_NAME_AND_PHONE
...
#     if context.user_data.get('full_name') and context.user_data.get('phone_number'):
#         await update.message.reply_text(
#             f"Thank you, *{context.user_data['full_name']}*! 🙏\n\nPlease provide your *full, secure delivery address* and postcode. 🚚",
#             parse_mode='Markdown'
#         )
#         return config.GETTING_ADDRESS
#     else:
...
#         return await check_address_and_proceed(update, context, context.user_data['address'])
#     else:
#         await query.edit_message_text("Of course. Please provide your new secure delivery address. 📍")
#         return config.GETTING_ADDRESS
...
#     message_to_send = "👍 Thank you. One moment while I verify your delivery address... 🗺️"
#     if update.callback_query:
#         await update.callback_query.edit_message_text(message_to_send)
#     else:
#         await update.message.reply_text(message_to_send)
...
#         await context.bot.send_message(
#             chat_id=chat_id, 
#             text=f"We're so sorry, but you are about {distance:.1f} miles away, which is outside our secure delivery radius of {config.DELIVERY_RADIUS_MILES} miles. 😥"
#         )
#         return ConversationHandler.END

#     context.user_data['delivery_charge'] = 0 if distance <= config.FREE_DELIVERY_RADIUS_MILES else config.DELIVERY_CHARGE
#     delivery_message = f"🎉 Great news! You're about *{distance:.1f} miles* away and qualify for *FREE insured delivery*." if context.user_data['delivery_charge'] == 0 else f"Excellent, you're about *{distance:.1f} miles* away! A secure delivery charge of *£{config.DELIVERY_CHARGE:.2f}* will apply. 🚚"
    
#     await context.bot.send_message(
#         chat_id=chat_id, 
#         text=f"{delivery_message}\n\nLet's begin your selection! ✨", 
#         parse_mode='Markdown'
#     )
#     return await show_menu(update, context)
...
#     """Displays the main menu categories."""
#     reply_markup = build_menu_keyboard()
#     message_text = "Please select a category below to begin your investment. 👇"
#     if update.callback_query:
#         await update.callback_query.answer()
#         try:
#             await update.callback_query.edit_message_text(message_text, reply_markup=reply_markup)
...
#     reply_markup = build_items_keyboard(category)
#     await query.answer()
#     await query.edit_message_text(f"Excellent choice. Here are our current offerings for *{category}*: ✨", reply_markup=reply_markup, parse_mode='Markdown')
#     return config.ORDERING
...
#             cart[item_name] = {'quantity': 1, 'price': item_details['price']}
#         context.user_data['cart'] = cart
#         await query.answer(f"✨ Added {item_name} to your selection!")
#         return await view_cart(update, context)
#     else:
#         await query.answer("My apologies, that item could not be found. 🔍", show_alert=True)
#         return config.ORDERING
...
#             del cart[item_name]
#         context.user_data['cart'] = cart
#         await query.answer(f"Removed one {item_name} from your selection.")
    
#     if not cart:
#         await query.answer("Your selection is now empty. 💎")
#         return await show_menu(update, context)
#     else:
#         return await view_cart(update, context)
...
#     if not cart:
#         await query.answer("Your selection is empty! 💎", show_alert=True)
#         return await show_menu(update, context)
...
#     )
#     intent = ai_response.get("intent")
#     reply_text = ai_response.get("reply", "Sorry, I didn't quite catch that. Could you please rephrase? 💬")
...
#             return await view_cart_from_text(update, context)
#         else:
#             await update.message.reply_text("I'm sorry, I couldn't find those items in our catalogue. 🔍")
...
#     cart = context.user_data.get('cart', {})
#     if not cart:
#         await update.message.reply_text("Your selection is empty! 💎", reply_markup=build_menu_keyboard())
#         return config.ORDERING
...
#     delivery_charge = context.user_data.get('delivery_charge', 0)
#     total_price = subtotal + delivery_charge
#     order_ref = f"NJ-{int(time.time())}" # NJ for Nutan Jewellers
#     context.user_data['order_ref'] = order_ref
...
#     payment_details = (
#         f"🧾 *Final Invoice*\n\n{summary}\n"
#         f"Insured Delivery: *£{delivery_charge:.2f}*\n"
#         f"---------------------\n*Total to Pay: £{total_price:.2f}*\n\n"
#         f"🏦 *Payment & Confirmation*\nPlease make a bank transfer to secure your order:\n\n"
#         f"  - *Account Name:* Nutan Jewellers Ltd\n"
#         f"  - *Sort Code:* 01-02-03\n"
#         f"  - *Account No:* 12345678\n"
#         f"  - *Reference:* `{order_ref}`\n\n"
#         f"After paying, please upload a *screenshot* of the confirmation as proof. 📄"
...
#     await query.answer()
#     await query.edit_message_text(payment_details, parse_mode='Markdown')
#     print(f"LOG: Invoice shown for {order_ref}. Transitioning to AWAITING_SCREENSHOT.")
#     return config.AWAITING_SCREENSHOT
...
#     order_details = db.get_order_details(order_ref)
#     if not order_details:
#         await update.message.reply_text("There was an issue finding your order. Please contact support. 🛠️")
#         return ConversationHandler.END

#     cart_summary, _ = get_cart_summary(order_details['cart'])
#     order_caption = (
#         f"🔔 *New Pending Order & Payment: #{order_ref}*\n\n"
#         f"👤 *Customer:* {order_details['name']}\n"
#         f"📞 *Phone:* {order_details['phone']}\n"
#         f"🏠 *Address:* {order_details['address']}\n\n"
#         f"{cart_summary}\n\n"
...
#     # This variable name is kept for logical consistency as requested
#     restaurant_bot = context.application.bot_data['restaurant_bot']
#     await restaurant_bot.send_photo(
#         chat_id=config.RESTAURANT_CHAT_ID, 
#         photo=open(screenshot_path, 'rb'), 
...
#     await update.message.reply_text("Thank you! ✨ Your order and payment have been sent to our team for confirmation. You'll be notified shortly. ⏳")
#     print(f"LOG: Order {order_ref} forwarded to business group. User in PENDING_CONFIRMATION.")
#     return config.PENDING_CONFIRMATION

# async def handle_text_instead_of_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
#     """Handles the user sending text when a screenshot is expected."""
#     await update.message.reply_text("I am awaiting your payment confirmation screenshot. Please upload an image to proceed. 🖼️ You can also type /cancel to end this transaction.")
#     return config.AWAITING_SCREENSHOT

# async def pending_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
#     """Informs the user their order is still pending."""
#     await update.message.reply_text("Your order is currently being verified by our team. We appreciate your patience and will notify you as soon as it's confirmed! 😊")
#     return config.PENDING_CONFIRMATION

# async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
#     """Cancels the entire conversation."""
#     await update.message.reply_text("Transaction cancelled. We hope to assist you with your investments again soon! 👋")
#     context.user_data.clear()
#     print("\n--- CONVERSATION CANCELLED ---\n")
#     return ConversationHandler.END
...
def build_menu_keyboard():
    ...
    menu = get_menu_as_dict()
    if not menu: return None
    # ✅ UPDATED: Changed emoji for Gold Coins to be more specific
    emoji_map = {
        "GOLD BARS": "💰",
        "GOLD COINS": "💰",
    }
...
def build_items_keyboard(category):
    ...
    menu = get_menu_as_dict()
    items = menu.get(category, [])
    # ✅ UPDATED: Uses the short 'itemid' for callback_data to prevent errors
    keyboard = [[InlineKeyboardButton(
        f"✨ {item['itemname']} - £{item['price']:.2f}",
        callback_data=f"add_{item['itemid']}"
    )] for item in items]
    keyboard.append([InlineKeyboardButton("⬅️ Back to Categories", callback_data="show_menu")])
    return InlineKeyboardMarkup(keyboard)
...
def build_cart_keyboard(cart):
    """Creates the interactive keyboard for the shopping cart."""
    keyboard = []
    # ✅ UPDATED: Uses the stored 'itemid' for the +/- buttons
    for item_name, details in cart.items():
        keyboard.append([
            InlineKeyboardButton("➖", callback_data=f"rem_{details['itemid']}"),
            InlineKeyboardButton(f"{details['quantity']}x {item_name}", callback_data="noop"),
            InlineKeyboardButton("➕", callback_data=f"add_{details['itemid']}")
        ])
    keyboard.append([InlineKeyboardButton("💳 Proceed to Checkout", callback_data="checkout")])
    keyboard.append([InlineKeyboardButton("🛍️ Continue Shopping", callback_data="show_menu")])
    return InlineKeyboardMarkup(keyboard)
...
async def add_item_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Adds an item to the cart via button press."""
    query = update.callback_query
    # ✅ UPDATED: Receives the short item_id from the button
    item_id = query.data.split("_", 1)[1]
    item_details = get_item_details(item_id)

    if item_details:
        # Use the full item_name as the key in the cart for display purposes
        ...
        if item_name in cart:
            ...
            # Store the itemid in the cart as well for later use (+/- buttons)
            cart[item_name] = {
                'quantity': 1,
...
async def handle_text_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if intent == "ADD_TO_ORDER":
        ...
        items_added = []
        for item_data in ai_response.get("items", []):
            # The AI will return a full name, which get_item_details can handle
            item_details = get_item_details(item_data.get("name"))
            if item_details:
                item_name = item_details['itemname']
                item_id = item_details['itemid']
                quantity = item_data.get("quantity", 1)
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the payment screenshot, saves it, and forwards to the admin bot."""
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action=constants.ChatAction.UPLOAD_PHOTO)
    print("LOG: Received photo in AWAITING_SCREENSHOT state.")
    photo = update.message.photo[-1]
    order_ref = context.user_data.get('order_ref')
    ...
    admin_bot = context.application.bot_data.get('restaurant_bot') # Using .get for safety
    if admin_bot:
